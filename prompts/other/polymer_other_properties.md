...
3. The "Conditions" Consolidation (`test_conditions`): The database uses a highly consolidated field for test conditions. You MUST synthesize the active medium and physics into this single string.
   - For Viscosity/Solubility: Extract the Solvent (e.g., "Solvent: CHCl3") and Concentration (e.g., "Concentration: 0.5 g/dL").
   - For Permeability: Extract the Gas and Upstream Pressure (e.g., "Gas: CO2, Pressure: 2 atm").
...
4. OCR CORRECTION & DATA CLEANING: 
   - Fix common PDF extraction errors (e.g., "1.6S" -> "1.65", "S0" -> "50").
   - DO NOT extract values vaguely "estimated from figure/graph" unless exact numbers are given.
5. Value & Unit Merging: The `value` field MUST contain BOTH the numeric value AND the unit (e.g., "45.2 %", "1.35 g/cm^3", "450 Barrer"). 
   - Qualitative Format: For Solubility, the value must be formatted strictly as semantic states: "Soluble", "Insoluble", or "Partially Soluble/Swollen".
6. Null Handling: Your output MUST contain exactly the 10 keys defined below. If a field is not applicable, output `null`. Do not omit keys.

### TARGET PROPERTIES (`record_type` ENUM):
You must classify every extracted property EXACTLY into one of the following strings based on its physical nature:
* Structure: "Crystallinity", "Density"
* Viscosity: "Intrinsic Viscosity", "Dynamic Viscosity", "Melt Viscosity"
* Solubility: "Solubility"
* Sorption: "Water Absorption / Solvent Uptake"
* Gas Transport: "Gas Permeability", "Gas Separation Selectivity"
...
* `value` (String): MANDATORY. The numeric value ALONG WITH ITS UNIT (e.g., "0.85 dL/g", "35.2" for dimensionless selectivity), or the qualitative state ("Soluble").
