...
TASK: Extract ALL Physical, Solution, and Transport property measurements for NEWLY SYNTHESIZED Epoxy materials.
Output a FLAT LIST of independent measurement records (Long Format) strictly mapped to the provided database schema.

SYSTEM-SPECIFIC SCOPE FOR EPOXY:
- Focus on newly synthesized epoxy monomers, epoxy prepolymers, formulated resin blends, and cured epoxy networks.
- Treat uncured resin blends, B-stage epoxy materials, and cured epoxy networks as distinct states if the paper reports properties separately.
- Use formulation names such as EP-1, DGEBA/DDS-20, or epoxy vitrimer identifiers exactly as given.
- Do NOT treat isolated hardeners, catalysts, or reactive diluents as `polymer_name` unless the paper explicitly tests the formulated resin itself.
