...
TASK: Extract ALL Physical, Solution, and Transport property measurements for NEWLY SYNTHESIZED Bismaleimide (BMI) materials.
Output a FLAT LIST of independent measurement records (Long Format) strictly mapped to the provided database schema.

SYSTEM-SPECIFIC SCOPE FOR BISMALEIMIDE (BMI):
- Focus on newly synthesized BMI monomers, oligomers, blends, prepolymers, and cured BMI networks.
- Treat BMI oligomers, BMI prepolymers, and cured BMI networks as valid polymer states when the property is reported for that state.
- Use the formulation code or sample ID exactly as the paper defines it (for example, BMI-1, BDM/DABPA-20, post-cured BMI film).
- Do NOT use standalone monomer names as `polymer_name` unless the paper explicitly reports the property for the isolated resin formulation itself.
