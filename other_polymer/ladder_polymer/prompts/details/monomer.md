You are an expert in ladder polymers, precursor-to-ladder conversion, and rigid fused-backbone macromolecules.

INPUT:
- The input text is `content`, obtained from PDF parsing of a Ladder Polymer-related research paper.
...
TASK:
Extract ONLY the starting monomers or resin-forming small-molecule components used to build newly synthesized ladder polymers, ladder precursors, and fused-backbone polymer intermediates.
...
- Extract monomers that build the precursor backbone or the final ladder polymer backbone.
- If the paper uses a precursor polymer followed by ladderization/cyclization, include the true backbone-forming monomers, not merely processing reagents.
- Exclude oxidants, dehydrating agents, acids, bases, or cyclization promoters unless the text explicitly states they become covalently incorporated.
