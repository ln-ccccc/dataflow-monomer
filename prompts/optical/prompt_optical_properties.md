...
* "Excitation Wavelength (λ_ex)": The peak excitation wavelength from excitation spectra. Look for "λ_ex", "excitation peak", "excitation band". Report the wavelength at which the emission is monitored in the `wavelength` field (e.g., "Monitored at λ_em = 385 nm").
* "Fluorescence Quantum Yield (Φ)": The ratio of photons emitted to photons absorbed. Look for "quantum yield", "QY", "Φ", "ΦF", "PLQY", "absolute quantum yield". Value is dimensionless (e.g., "43.9 %" or "0.439"). CRITICAL: Distinguish between absolute quantum yield (measured with integrating sphere) and relative quantum yield (measured against a standard). Record the method (integrating sphere vs. relative) in `test_method`. If measured in solution, record solvent and concentration in `thickness`. If measured as pure/neat/solid sample, note "Pure/Neat" in `test_conditions`.
* "Fluorescence Lifetime (τ)": The average time a molecule stays in the excited state before emitting a photon. Look for "fluorescence lifetime", "τ", "decay time", "PL lifetime". Value includes unit (e.g., "0.85 ns", "4.29 ns"). Often measured by time-correlated single photon counting (TCSPC) or transient-state fluorescence spectroscopy.
